...
class StockScreener:
    ...
    def create_pabbly_html(self, alerts: List[StockAlert]) -> str:
        """Create complete HTML email for Pabbly (no Iterator needed)"""

        # Build table rows
        rows_html = ""
        for alert in alerts:
            drop_class = "drop-extreme" if alert.drop_pct <= -80 else "drop-high" if alert.drop_pct <= -50 else "drop-moderate"
            rows_html += f"""
                    <tr>
                        <td class="ticker">{alert.ticker}</td>
                        <td class="price-current">${alert.current_price:.2f}</td>
                        <td class="price-ath">${alert.ath_5y:.2f}</td>
                        <td class="drop {drop_class}">{alert.drop_pct:.1f}%</td>
                        <td class="date-text">{alert.ath_date}</td>
                    </tr>"""
...
<head>
    ...
    <style>
        ...
        .count-badge {{ display: inline-block; background-color: #dc3545; color: white; padding: 5px 15px; border-radius: 20px; font-weight: bold; font-size: 16px; }}
        .content {{ padding: 0 20px 20px 20px; }}
        .section-title {{ color: #333; font-size: 20px; font-weight: 600; margin: 20px 0 15px 0; padding-bottom: 10px; border-bottom: 2px solid #667eea; }}
        table {{ width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 14px; }}
        thead {{ background-color: #667eea; color: white; }}
        th {{ padding: 12px 10px; text-align: left; font-weight: 600; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; }}
        td {{ padding: 12px 10px; border-bottom: 1px solid #e9ecef; }}
        tr:hover {{ background-color: #f8f9fc; }}
        .ticker {{ font-weight: bold; color: #667eea; font-size: 15px; }}
        .price-current {{ color: #28a745; font-weight: 600; }}
        .price-ath {{ color: #6c757d; }}
        .drop {{ font-weight: bold; font-size: 15px; }}
        .drop-extreme {{ color: #dc3545; }}
        .drop-high {{ color: #fd7e14; }}
        .drop-moderate {{ color: #ffc107; }}
        .date-text {{ color: #6c757d; font-size: 13px; }}
        .footer {{ background-color: #f8f9fc; padding: 20px; text-align: center; color: #6c757d; font-size: 13px; border-top: 1px solid #e9ecef; }}
        .footer strong {{ color: #495057; }}
        .disclaimer {{ margin-top: 15px; padding: 12px; background-color: #fff3cd; border-left: 4px solid #ffc107; border-radius: 4px; font-size: 12px; color: #856404; text-align: left; }}
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="summary">
            <div class="summary-item"><strong>🗓️ Date:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>
            <div class="summary-item"><strong>🔔 Total Alerts:</strong> <span class="count-badge">{len(alerts)}</span> stocks found</div>
        </div>
...
{rows_html}
        ...
        <div class="footer">
            <p><strong>🤖 Automated Stock Screener Report</strong></p>
            <p>Generated by Railway · Data from Alpaca Markets API · Delivered via Pabbly Connect</p>
            <div class="disclaimer">
                ⚠️ <strong>Disclaimer:</strong> This is an automated notification for informational purposes only.
                Stocks showing significant drops from their all-time highs may represent buying opportunities or continued downtrends.
    ...
    def send_to_pabbly(self, alerts: List[StockAlert]) -> bool:
        ...
        try:
            ...
            # Generate complete HTML email (no Iterator needed in Pabbly!)
            html_email = self.create_pabbly_html(alerts)
            ...
            payload = {
                ...
                "html_email": html_email,  # Complete HTML ready to send!
                "alerts": [
                    {
                        "ticker": alert.ticker,
